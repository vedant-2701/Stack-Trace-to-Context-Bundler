...
- [ ] Given a trace with a `dependency`-bucket frame whose full path
      exactly matches a `packages` entry in a valid lockfileVersion
      2/3 `package-lock.json`, when resolved, then `Locked[pkg].Version`
      is set from that entry with no `Note`.
- [ ] Given a `dependency`-bucket frame under a scoped package (e.g.
...
- [ ] Given a `dependency`-bucket frame's exact nested path has no
      ...
      match.
- [ ] Given two `dependency`-bucket frames sharing the same
...
- [ ] Given two `dependency`-bucket frames sharing the same
      ...
      *other* matches neither exactly nor via fallback, when resolved,
      ...
      resolution (requirement 12).
- [ ] Given a package with neither an exact nor top-level match in a
      valid, present lockfile, when resolved, then `Locked[pkg].Version`
      is omitted and `Note` explains no match was found.
- [ ] Given `package-lock.json` is entirely absent (but `package.json`
      exists and parses), when resolved, then every referenced
      package's `Locked` entry has `Version` omitted and a `Note`
      explaining no lockfile was found.
- [ ] Given only `yarn.lock` (no `package-lock.json`) is present, when
      resolved, then this is treated identically to "no lockfile found"
      (previous criterion) — not a crash, not a silent skip.
- [ ] Given `package-lock.json` exists but is not valid JSON, when
      resolved, then every referenced package's `Locked` entry has
      `Version` omitted and a `Note` explaining the parse failure.
- [ ] Given `package-lock.json`'s `lockfileVersion` is `1`, when
      resolved, then every referenced package's `Locked` entry has
      `Version` omitted and a `Note` explaining the unsupported format
      — not a crash, not an attempt to parse it as v2/v3.
- [ ] Given `package.json` cannot be found at repo root, when the
      bundle is built, then `Bundle.Dependencies` is nil and omitted
      from the JSON entirely.
- [ ] Given `package.json` exists but is not valid JSON, when the
      bundle is built, then `Bundle.Dependencies` is nil, same as the
      not-found case.
- [ ] Given the tool is run outside any git working tree (no `.git`
      anywhere) but `package.json` exists directly in `workDir`, when
      the bundle is built, then `Bundle.Dependencies` is populated using
      `workDir` as the root, same as if git had reported it.
- [ ] Given the tool is run outside any git working tree and
      `package.json` does **not** exist in `workDir` either, when the
      bundle is built, then `Bundle.Dependencies` is nil — not a panic,
      not a crash of the whole bundle.
- [ ] Given git detection succeeds but the reported repo root has no
