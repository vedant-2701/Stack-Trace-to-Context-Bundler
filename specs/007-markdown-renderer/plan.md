...
├── escape_test.go
├── fixtures_test.go  one contract.Bundle-building func per golden case
│                     below -- hand-authored contract.Bundle{...} Go
│                     literals for every case including ts_basic, no
│                     dependency on 001's own generated
│                     example_ts.json fixture
└── testdata/
    └── golden/
        ├── ts_basic.golden.md
...
- **Golden-file tests** (`markdown_test.go`), one per fixture in the table
  ...
  function and its golden path, per `CONVENTIONS.md`. A `go test
