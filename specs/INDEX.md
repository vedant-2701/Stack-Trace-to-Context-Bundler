...
| 002b | Pipeline wiring | Wires 003a/003b–009 together into the full detect → parse → render → clipboard flow | 002a, 003a, 003b, 004, 006a, 007, 008, 009 | idea |
...
| 003b | Language auto-detection registry | Tries each registered parser's `Detect()` against the raw trace; defines "no match"/ambiguous behavior. Needs real parsers to test detection heuristics against, not just the interface shape. | 001, 003a, 006a | done |
...
| 008 | JSON renderer | Bundle → compact, HTML-unescaped, no-trailing-newline contract JSON | 001 | done |
| 009 | Clipboard integration | OS-appropriate clipboard write via subprocess (`pbcopy` on macOS; WSL detected and bridged to `clip.exe` exclusively, else `wl-copy` → `xclip` runtime-failure fallback chain on Linux, clear stderr error if neither is available; `clip.exe` on Windows) | — | in-progress |
