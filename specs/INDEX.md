...
| 009 | Clipboard integration | OS-appropriate clipboard write via subprocess (`pbcopy` on macOS; WSL detected and bridged to `clip.exe` exclusively, else `wl-copy` → `xclip` runtime-failure fallback chain on Linux, clear stderr error if neither is available; `clip.exe` on Windows) | — | done |
