...
| 004 | Own-code context extraction | Shared: read file at line, snippet window, `git blame -L`, repo-level git metadata (current commit, branch, uncommitted changes), stale/not-found handling — language-agnostic | 001 (+ requires a `001` contract patch: `GitMetadata` → pointer, schemaVersion MAJOR bump) | in-progress |
