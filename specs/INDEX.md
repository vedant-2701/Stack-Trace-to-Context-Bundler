...
| 003b | Language auto-detection registry | Tries each registered parser's `Detect()` against the raw trace; defines "no match"/ambiguous behavior. Needs real parsers to test detection heuristics against, not just the interface shape. | 001, 003a, 006a | in-progress |
