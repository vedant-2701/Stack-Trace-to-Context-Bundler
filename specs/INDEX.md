...
| 006b | TS/JS dependency resolution | `package.json`+lockfile resolution for `dependency`-bucket frames | 001 (+ requires two `001` contract patches: `Dependencies` → pointer + `LockedDependency` note-comment relaxation, schemaVersion MAJOR bump), 006a | in-progress |
