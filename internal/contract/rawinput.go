...
// bytes, the longest a UTF-8 rune can be) to the last complete rune
// boundary at or before the cap, rather than emit a string ending in an
// invalid trailing byte sequence. The result can therefore be up to 3
// bytes under rawInputCapBytes in that case -- an accepted trade-off:
