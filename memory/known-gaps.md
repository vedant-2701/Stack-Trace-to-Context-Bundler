...
| 003a-language-parser-interface | `LanguageParser.Language()` is a static per-implementation method returning one fixed `contract.Language`. TypeScript compiles to `.js` before running, so a compiled-then-`node`-run trace may be structurally indistinguishable from plain JavaScript -- verify whether a static return is actually sufficient for the `typescript` package, or whether language must be determined per-trace (e.g. returned from `Parse()` instead) once real `ts-node`/`tsx`/`bun`/`deno`/compiled-`node` traces are examined | 006a | pending -- design question itself answered during 006a interrogation (two static `LanguageParser` values, `Detect()` splits per-trace; see spec.md FR5); box stays open until 006a is fully implemented and its own spec.md acceptance criteria are checked off |
