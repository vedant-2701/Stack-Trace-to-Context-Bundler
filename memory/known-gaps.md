...
| 003b-language-auto-detection-registry | `DetectLanguage`'s ambiguous (2+ match) branch is proven correct only via hand-written fakes (T004) -- no real fixture can exercise it, since `javascriptParser`/`typescriptParser` (006a) are constructed to never both match the same real trace | 005a | pending |
