// Command stack-trace-bundler-all is the stack-trace-bundler CLI
// entrypoint that will eventually auto-detect Java vs. TypeScript/
// JavaScript traces. For now (002a), it only reads and validates CLI
// input -- see specs/002a-cli-input-handling/spec.md. The real
// detect -> parse -> render -> clipboard pipeline, and real stdout
...
func main() {
	input, verbosity, err := cli.ParseAll(os.Args[1:], os.Stdin, stdinIsPiped())
	if err != nil {
	...
	// section (T006c) for why logging this during cli.ParseAll itself,
	// before the level was known, silently dropped the message under
	// every verbosity setting.
...
// os.Stdin.Stat() and passed into cli.ParseAll as a plain bool -- keeps
// internal/cli's core logic pure and testable with fake readers, no real
// TTY needed in tests, per plan.md's Architecture section.
func stdinIsPiped() bool {
	stat, err := os.Stdin.Stat()
	if err != nil {
		// Can't determine TTY state -- fail safe toward "not piped" so
		// cli.ParseAll's no-input fast-fail (spec requirement 6) kicks
		// in, rather than risking a block on a read that might never
		// complete.
		return false
	}
